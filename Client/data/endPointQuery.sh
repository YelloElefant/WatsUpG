...
response=$(curl -f -X POST -d 'token='123456789'' http://$serverPrivateIp:2525/Api/UpOrDown.php)

if [ -z $response ]; then
  echo "private ip is empt"
  response2=$(curl -f -X POST -d 'token='123456789'' https://$serverDomainName/Api/UpOrDown.php)
  echo $response2
  if [ $response2 = 'up' ]; then

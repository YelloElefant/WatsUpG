...
response=$(curl --connect-timeout 1 -f -X POST -d 'token='123456789'' http://$serverPrivateIp:2525/EndPointQuery.php)

if [ -z $response ]; then 
  echo "private ip is empt"; 
  response2=$(curl --connect-timeout 1 -f -X POST -d 'token='123456789'' https://$serverDomainName/EndPointQuery.php)
  echo $response2
  if [ $response2 = 'up' ]
  then
